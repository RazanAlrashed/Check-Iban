...
function vildationIban(iban) 
{
    if(!iban) //check if iban is empty
    {
        return false;
    }

    iban=iban.trim(); // remove spacec from iban
    iban=iban.replace(/\s=+/g,'');//remove spacec from center iban
    if(iban.length !=24)
    {
        return false;
    }
...
{
    const iban=document.getElementById("s1").value;

    if (!vildationIban(iban))
    {
        var msg=document.getElementById('showmassage');
        msg.innerHTML="your iban is not coreect";
    }
    ...
    else
    {
        ...
        const bankname=banks.find(b=>b.samaCode===samacode)
        var msg=document.getElementById('showmassage');
        msg.innerHTML=bankname.nameEn;
        var sama=document.getElementById('showsamacode')
        sama.innerHTML=bankname.samaCode
    }
} 